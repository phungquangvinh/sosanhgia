...
function windowPrompt(data){
	...
	// Width, Height temp
	widthTemp	= 18;
	heightTemp	= (wPromptOpts.showBottom ? 52 : 18);
				...
				if(wPromptOpts.iframe && wPromptOpts.href != null) html += wPromptOpts.content;
					...
					if(wPromptOpts.confirm != false){
						html += '<div class="wPromptConfirmButton">';
							html += '<input type="button" class="wPromptInputButton ' + optsConfirm.classTrue + '" value="' + optsConfirm.valueTrue + '" onclick="confirmWindowPrompt(true)" /> &nbsp;';
							html += '<input type="button" class="wPromptInputButton ' + optsConfirm.classFalse + '" value="' + optsConfirm.valueFalse + '" onclick="confirmWindowPrompt(false)" />';
						html += '</div>';
					}
					html += '</div>';
				}
	...
	$("body").prepend(domEle);
	wPromptDomEle	= $(document.getElementById("wPrompt"));
