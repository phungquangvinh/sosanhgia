...
class fsDataGird
{
   ...
   var $classButton        =  'btn btn-info btn-search';
   ...
   function setSelectSort(){
	...
	function showHeader($total_list){
      ...
      $str .=  '<div class="content">';
		// khoi tao table
		$str .= '<table class="table table-bordered" id="listing" cellpadding="5" cellspacing="0" border="1" bordercolor="' . $this->fs_border . '" width="100%" class="table">';
	...
	*/
	...
	function checkType($row,$key,$check_action = true){
		...
		switch($this->arrayType[$key]){
			...
			//kiểu tiền tệ VNĐ
			case "vnd":
				return '<td ' . $this->arrayAttribute[$key] . ' style="text-align: right;"><span class="vnd" style="display:inline" id="' . $this->arrayField[$key] . ',' . $row[$this->field_id] . ',3">' .  format_number($row[$this->arrayField[$key]]) . '</span></td>';
			break;
         ...
         case "datetime":
				return '<td class="date" ' . $this->arrayAttribute[$key] . '>' .  date("d/m/Y H:i:s",$row[$this->arrayField[$key]]) . '</td>';
			break;
...
<td ' . $this->arrayAttribute[$key] . ' class="tooltip"  title="' . translate_text("Click vào để sửa đổi sau đó enter để lưu lại") . '">
	...
	*/
	function addSearch($name,$field,$type,$value = '',$default="",$add_html = ""){
		...
		switch($type){
			//kiểu array
			case "array":
				$str .= '<select name="' . $field . '" id="' . $field . '" class="textbox" '.$add_html.'>';
					foreach($value as $id=>$text){
						$str .= '<option value="' . $id . '" ' . (($default==$id) ? 'selected="selected"' : '') . '>' . $text . '</option>';
					}
				$str .= '</select>';
			break;
			...
			//kiểu ngày tháng
			case "date":
				$str .= '<input '.$add_html.' type="text"  class="textbox" name="' . $field . '" id="' . $field . '"  onKeyPress="displayDatePicker(\'' . $field . '\', this);" onClick="displayDatePicker(\'' . $field . '\', this);"  value="' . $value . '">';
			break;

			//kiểu text box
			case "text":
				$value  = getValue($field,"str","GET", "");
				$str .= '<input onkeydown="submit_form(event,this);" '.$add_html.' type="text"  class="textbox" name="' . $field . '" id="' . $field . '"  value="' . $value . '">';
			break;
         // Kiểu checkbox
         case "checkbox":
            $value   =  getValue($field,"int","GET",0);
            $str  .= '<input '.$add_html.' type="checkbox" name="' . $field . '" id="' . $field . '" value="1" />';
         // Kiểu hidden
			case "hidden":
		...
		$this->arrayAddSearch[] = array($str, $field, $name, $type);
	}
   /**
   ...
   function showButtonSearch(){
      return '<td style="padding: 3px;"><input type="submit" class="' . $this->classButton . '" value="' . translate_text("Tìm kiếm") . '"></td>';
   }
	...
	*/
	function urlsearch(){
		$str   =  '';
		$i     =  0;
      $open_td    =  '<td valign="top"><table cellpadding="3">';
      $close_td   =  '</table></td>';
		...
		foreach($this->arraySearch as $key=>$field){
			...
			switch($this->arrayType[$key]){
				...
				case "text":
					...
					$str .= '<td class="label_text">&nbsp;'.$this->arrayLabel[$key].'&nbsp;:&nbsp;</td><td><input type="text" class="textbox" name="' . $field . '" id="' . $field . '" onfocus="if(this.value==\'' . $this->arrayLabel[$key] . '\') this.value=\'\'" onblur="if(this.value==\'\') this.value=\'' . $this->arrayLabel[$key] . '\'" value="' . $value . '"></td>';
				break;
				case "numbernotedit":
					$value = getValue($field,"str","GET",$this->arrayLabel[$key]);
