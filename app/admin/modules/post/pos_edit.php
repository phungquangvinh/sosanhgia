...
$updateListPost = Post::where('pos_id = '.$id)->first();

if ($action == 'edit' && $updateListPost) {
    $rules = [
    ...
    $post_back_id = $updateListPost->update([
        'pos_title' => $pos_title,
        'pos_slug' => str_slug($pos_title),
        'pos_teaser' => $pos_teaser,
        'pos_image' => isset($posImage) ? $posImage : $updateListPost->pos_image,
...
echo $template->render('edit', compact('load_header', 'updateListPost', 'pos_category', 'pos_author'));
